...
exports.login = async (req, res, next) => {
    try {
        ...
        if(foundUser.email_verified_at == null) {
            throw new Error("User email is not validated");
        }
...
exports.validateEmailToken = async (req, res, next) => {
    ...
    try {
        ...
        if(emailToken) { 
            ...
            throw new Error("Email already validated or not found");
        }
...
exports.sentAccountValidationEmail = async (req, res, next) => {
    ...
    try {
        ...
        if(elapsedTime < minTime) {
            throw new Error(`At least ${minTime / 60} minutes must pass before another request`);
        }
    ...
    try {
      ...
      if (!passwordResetToken) {
        ...
        if (currentTimestamp <= tokenExpiryTimestamp) {
          throw new Error('There is already a request for this email. Please check your email.');
        }
        await passwordResetToken.update({
            reset_token: resetToken,
            user_id: findUser.id,
            token_expiration_date: resetTokenExpiry}, { transaction: t });
      }
